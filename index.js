...
import { consultarPartes } from "./scraper_muni_lascondes.js";
import { enviarAlerta } from "./mailer.js";
import { readFileSync, writeFileSync, existsSync } from "fs";

// ===================== CONFIGURACIÓN =====================

const PLACA = process.env.PLACA;
...
async function alerta(nuevasMultas) {
  ...
  try {
    ...
    log("📧", "Correo enviado a " + process.env.CORREO);
  } catch (err) {
...
console.log(
  `  Modo:   ${IS_CI ? "GitHub Actions" : `Local (${CRON_SCHEDULE})`}`
);
console.log(`  ${sep}`);
